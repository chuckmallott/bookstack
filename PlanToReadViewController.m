...
@interface PlanToReadViewController ()

@end
NSMutableArray *planToRead;

@implementation PlanToReadViewController
...
- (void) populateList {
    ...
    NSData *dataRepresentingSavedArray = [currentDefaults objectForKey:@"planToRead"];
    if (dataRepresentingSavedArray != nil)
    {
        NSArray *oldSavedArray = [NSKeyedUnarchiver unarchiveObjectWithData:dataRepresentingSavedArray];
        if (oldSavedArray != nil)
        {
            planToRead = [[NSMutableArray alloc] initWithArray:oldSavedArray];
        }
        else
        {
            planToRead = [[NSMutableArray alloc] init];
        }
    }
    float height = planToRead.count * 80;
    if (height < 80.0f)
    {
        height = height + 80.0f;
    }
    ...
    if(planToRead.count > 0)
    {
        ...
        UIView *removeView  = [self.view viewWithTag:104];
        [removeView removeFromSuperview];
        removeView  = [self.view viewWithTag:105];
        [removeView removeFromSuperview];
        removeView  = [self.view viewWithTag:106];
        [removeView removeFromSuperview];
    }
    else {
        ...
        labelSmall.text = @"Add books you're planning to read to this stack.";
        labelSmall.textAlignment = NSTextAlignmentCenter;
        labelSmall.numberOfLines = 0;
        labelSmall.tag = 105;
        [self.view addSubview:labelSmall];
        
        UIImage *image = [UIImage imageNamed:@"mvp-assets/icon-planning-large.png"];
...
- (void)viewDidLoad {
    ...
    titleBlock.backgroundColor = [UIColor colorWithRed:105/255.0 green:119/255.0 blue:127/255.0 alpha:.10];
    ...
    titleBlockText.text = @"PLANNING TO READ";
    titleBlockText.numberOfLines = 0;
    [self.view addSubview:titleBlockText];
    
    UIImage *stackIcon = [UIImage imageNamed:@"mvp-assets/icon-planning-small-grey.png"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    viewController.book = [planToRead objectAtIndex:indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)thistableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SCBook *book = [planToRead objectAtIndex:indexPath.row];
    if (cell == nil) {
